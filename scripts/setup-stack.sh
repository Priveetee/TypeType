...
DEFAULT_POSTGRES_DB="typetype"
DEFAULT_POSTGRES_USER="typetype"
DEFAULT_POSTGRES_PASSWORD="typetype"
DEFAULT_DRAGONFLY_URL="redis://dragonfly:6379"
DEFAULT_GITHUB_REPO="TypeType-Video/TypeType"
...
if [[ -f "${ENV_FILE}" ]]; then
  ...
  if [[ ! "${overwrite}" =~ ^[Yy]$ ]]; then
    ...
    prompt_value POSTGRES_DB "POSTGRES_DB" "${DEFAULT_POSTGRES_DB}"
    prompt_value POSTGRES_USER "POSTGRES_USER" "${DEFAULT_POSTGRES_USER}"
    prompt_value POSTGRES_PASSWORD "POSTGRES_PASSWORD" "${DEFAULT_POSTGRES_PASSWORD}"
